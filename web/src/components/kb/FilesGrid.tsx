    ...
    >
      ...
      {gridDragOver && (
        <div className="absolute inset-0 z-40 bg-background/80 backdrop-blur-sm flex items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center gap-3 border-2 border-dashed border-primary rounded-xl px-12 py-10">
            ...
            <p className="text-xs text-muted-foreground">至 {currentPath === '/' ? '文件' : currentPath.replace(/\/$/, '').split('/').pop()}</p>
          </div>
        </div>
      )}
      ...
      <div className="shrink-0 flex items-center gap-1.5 px-4 h-10 border-b border-border">
        ...
        {isBrowsing ? (
          ...
          <>
            <button onClick={() => { /* TODO: trigger search in PDF viewer */ }} className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-accent rounded-md transition-colors cursor-pointer" title="文档内搜索">
              <Search className="size-3.5" />
            </button>
      ...
      <ContextMenuTrigger asChild>
        ...
        >
          ...
          <div className="px-2 py-1.5 flex flex-col gap-0.5">
            ...
            {doc.status === 'failed' && <span className="text-[9px] font-medium text-destructive/80">失败</span>}
          </div>
        </div>
      </ContextMenuTrigger>
      <ContextMenuContent>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-6">
      <div className="text-center">
        ...
        <p className="text-xs text-muted-foreground/60 mt-1">上传文档或创建笔记以开始使用</p>
      </div>
