...
export function GraphViewer({ kbId, focusNodeId, onNavigateToDoc }: Props) {
  ...
  const handleRebuild = React.useCallback(async () => {
    ...
    try {
      ...
      if (total === 0) {
        toast.info('未找到引用或交叉引用')
      } else {
        toast.success(`${res.citations} 个引用，${res.links} 个交叉引用`)
      }
      fetchGraph()
    } catch {
      toast.error('重建引用失败')
  ...
  if (loading) {
    ...
    overlay = <p className="text-sm text-muted-foreground">加载图谱数据失败</p>
  } else if (!hasNodes) {
    overlay = <p className="text-sm text-muted-foreground">暂无可文档化</p>
  } else if (!hasEdges) {
    overlay = (
      <div className="flex flex-col items-center gap-3">
        <p className="text-sm text-muted-foreground text-center max-w-xs">
          找到 {graphData.nodes.length} 个文档，但尚未索引任何引用。
        </p>
        <button
